...
import { catchAsync, AppError } from "../../lib/errors.js";
...
export function createBlogController({ service, Blog, anthropicApiKey }) {
  ...
  const generateCoverImage = catchAsync(async (req, res) => {
    ...
    const subject = words.slice(0, 6).join(" ") || "physiotherapy";
    const prompt = `Professional health and wellness photography, physiotherapy and rehabilitation, ${subject}, bright clean clinic setting, editorial style, soft natural light, wide shot, no text, no words, no letters, no watermarks, no labels`;
  ...
  const improveContent = catchAsync(async (req, res) => {
    ...
    const systemPrompt = `You are a professional editor for a physiotherapy blog written by a practising physiotherapist. Your job is to rewrite the HTML content you receive so it reads like it was written by an experienced, confident clinician, not by AI.
...
6. Maximum 2 to 3 sentences per <p> tag. If a paragraph is longer, split it into two <p> tags.
...
9. British English spelling (colour, recognise, specialise).
...
11. Be specific and clinically accurate. Replace vague phrases with concrete detail (a movement, a muscle group, a timeframe, a rep range) where the meaning allows. Do not invent medical claims.
12. Keep the tone warm and reassuring for patients. Avoid alarmist language.
13. Do not start the content with an <h2>. The first element must be a <p>.`;
